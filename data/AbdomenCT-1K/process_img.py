...
def resamplig(nibimg, header, niblabel, header_label):
    ...
    label_resampled = resample_mask(niblabel, header_label, final_space, 4)
...
def save(path, subjectid, data_cropped, label_cropped):
    ...
    new_path = path[subjectid].replace('AbdomenCT', 'AbdomenCT-propressed').replace(file_name, '')
    os.makedirs(new_path, exist_ok=True)
    new_path_label = path[subjectid].replace('AbdomenCT', 'AbdomenCT-propressed').replace('/img/', '/label/').replace(file_name, '')
