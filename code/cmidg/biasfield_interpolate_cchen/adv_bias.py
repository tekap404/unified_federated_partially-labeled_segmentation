...
def bspline_kernel_3d(sigma=[1, 1, 1], order=2, asTensor=False, dtype=torch.half, device='gpu'):
    '''
    ...
    :return:
    '''
    kernel_ones = torch.ones(1, 1, *sigma).cuda(device)
    kernel = kernel_ones
    ...
    if asTensor:
        return kernel[0, 0, ...].cuda(device)#.to(dtype=dtype, device=device)
    else:
...
class AdvBias(AdvTransformBase):
    ...
    def init_config(self,config_dict):
        ...
        self.gpu_id=config_dict['gpu_id']
    ...
    def init_parameters(self):
        ...
        self._device = self.gpu_id#'cuda' if self.use_gpu else 'cpu'
...
https://github.com/airlab-unibas/airlab/blob/1a715766e17c812803624d95196092291fa2241d/airlab/transformation/pairwise.py
        ...
        self.param = self.unit_normalize(self.param, p_type ='l2')
        self.param = self.param.cuda(self._device)#.to(dtype=self._dtype, device=self._device)
    ...
    def get_bspline_kernel(self,spacing, order=3):
        ...
        self._kernel.unsqueeze_(0).unsqueeze_(0)    # add channel for bs,c
        self._kernel = self._kernel.cuda(self._device)#.to(dtype=self._dtype, device=self._device)
        return self._kernel
    def get_name(self):
        return 'bias'
